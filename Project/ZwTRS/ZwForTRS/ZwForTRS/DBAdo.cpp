...
bool CDBAdo::SetConnectionString(CString strDriver, CString strIP, CString strCatalog, CString strUserID, CString strPassword)
{
	...
	//构造连接字符串
	m_strConnect	= strProvider+strDataSrc+strData+strUID+strPWD+_T("Persist Security Info=True;");
...
bool CDBAdo::OpenRecordset(CString szSQL)
{
	try
	{
		//关闭记录集
		...
		m_ptrRecordset->Open((_variant_t)szSQL, m_ptrConnection.GetInterfacePtr(), adOpenDynamic, adLockOptimistic, adCmdText);
...
void CDBAdo::AddParamter(LPCTSTR lpcsrName, ADOCG::ParameterDirectionEnum Direction, ADOCG::DataTypeEnum Type, long lSize, _variant_t & vtValue)
{
