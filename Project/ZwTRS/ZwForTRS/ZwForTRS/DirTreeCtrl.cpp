...
LPCTSTR CDirTreeCtrl::GetSubPath(LPCTSTR strPath)
{
	...
	if (iPos != -1)
	{
		CString strTmp = strTemp.Right(4);
		if (strTmp.CompareNoCase(_T(".dwg")) == 0)
		{
			int nCount = strTemp.GetLength() - iPos - 5;
...
void CDirTreeCtrl::DisplayPath(HTREEITEM hParent, LPCTSTR strPath)
{
	...
	while(bFind)
	{
		...
		if (!find.IsDirectory() && m_bFile)
		{
			...
			CString strTmp1 = strTmp.Right(4);
			if (strTmp1.CompareNoCase(_T(".dwg")) == 0)
			{
