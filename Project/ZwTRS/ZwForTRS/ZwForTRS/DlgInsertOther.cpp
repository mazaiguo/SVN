...
BOOL CDlgInsertOther::OnInitDialog()
{
	...
	m_strFileNameArr.RemoveAll();
	CString strAppPath = gGlobal.GetAppPath();
	CString strPath = strAppPath + _T("其它\\*.*");
	ArxDbgUtils::GetFileNameFromThePath(strPath, m_strAllFileNameArr, _T(".DWG"), true);
	...
	if (m_nCount>9)
	{
		...
		for (int i=0; i<9; i++)
		{
			...
			strTitle = ArxDbgUtils::GetFileNameByPath(m_strFileNameArr.GetAt(i));
