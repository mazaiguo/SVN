...
/////////////////////////////////////////////////////////////////////////////
BOOL CGlobal::SetVar(LPCTSTR szVName, ads_real rArg)
{
	struct resbuf res;
	res.restype = RTREAL;
...
/////////////////////////////////////////////////////////////////////////////
 BOOL CGlobal::SetVar(LPCTSTR szVName, ads_point pArg)
{
	struct resbuf res;
...
/////////////////////////////////////////////////////////////////////////////
 BOOL CGlobal::SetVar(LPCTSTR szVName, int nArg)
{
	struct resbuf res;
	res.restype = RTSHORT;
...
/////////////////////////////////////////////////////////////////////////////
 BOOL CGlobal::SetVar(LPCTSTR szVName, LPCTSTR szArg)
{
...
/////////////////////////////////////////////////////////////////////////////
 BOOL CGlobal::GetVar(LPCTSTR var, ads_real *rRes)
{
	ASSERT(rRes!=NULL);
	struct resbuf v;
...
/////////////////////////////////////////////////////////////////////////////
 BOOL CGlobal::GetVar(LPCTSTR var, ads_point *pRes)
{
	ASSERT(pRes != NULL);
	struct resbuf v;
	if(ads_getvar(var, &v) !=RTNORM)
...
/////////////////////////////////////////////////////////////////////////////
 BOOL CGlobal::GetVar(LPCTSTR var, int *nRes)
{
	ASSERT(nRes!=NULL);
	struct resbuf v;
	if(ads_getvar(var, &v)!=RTNORM)
...
/////////////////////////////////////////////////////////////////////////////
BOOL CGlobal::GetVar(LPCTSTR var, LPTSTR sRes)
{
	struct resbuf v;
...
CString CGlobal::GetCsvFile()
{
	CString path;
	CString strAppPath = GetAppPath();
	path = strAppPath + _T("data.txt");
	return path;
}
/////////////////////////////////////////////////////////////////////////////
//功 能：读取注册表读字符串的值
...
int CGlobal::GetRegister(HKEY hKey, 
								   LPCTSTR lpszSec, 
								   LPCTSTR lpszKey, 
								   LPCTSTR lpszDef, 
								   LPTSTR lpszValue)
...
/////////////////////////////////////////////////////////////////////////////
//读注册表,读DWORD的值
DWORD CGlobal::GetRegister(HKEY hKey,LPCTSTR lpszSec,LPCTSTR lpszKey,DWORD DefValue)
{
...
BOOL CGlobal::SetRegister(HKEY hKey,LPCTSTR lpszSec, LPCTSTR lpszKey, LPCTSTR lpszValue)
{
	HKEY hInfor;
	DWORD dwStatus;
...
/////////////////////////////////////////////////////////////////////////////
//功 能：写入注册表,写DWORD
BOOL CGlobal::SetRegister(HKEY hKey,LPCTSTR lpszSec,LPCTSTR lpszKey,DWORD Value)
{
	HKEY hInfor;
	DWORD dwStatus;			
	if(RegCreateKeyEx(hKey,
...
void CGlobal::setBlkRefCount(int nValue)
{
	AcDbObjectId StyleId;

	AcDbDictionary* testDict = ArxDbgUtils::openDictionaryForWrite(
		CBaseDataInDwg::dictName(), true,
...
int CGlobal::getBlkRefCount()
{
	int nBlkRefCount = 1;
	AcDbObjectId StyleId;
	AcDbDictionary* testDict = ArxDbgUtils::openDictionaryForRead(CBaseDataInDwg::dictName(), acdbHostApplicationServices()->workingDatabase());
	if (testDict)
	{
		Acad::ErrorStatus es;
		...
		if (testDict->getAt(_T("BASE"), StyleId) != Acad::eOk)
		{
			...
			if (es == Acad::eOk) 
			{
				nBlkRefCount = newRec->blkRefCount();
				newRec->close();
			}
			else 
			{
